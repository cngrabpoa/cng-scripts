/**
 * CNG OFFICIAL PRODUCT LOADER - ALL-IN-ONE
 * UI, CSS & Logic combined.
 */

(function() {
    // 1. INJECT ALL CSS STYLES
    const css = `
    ...
    // 2. INJECT LIGHTBOX HTML STRUCTURE
    const lbHTML = `
...
async function loadUltraProduct(pid) {
    ...
    try {
        ...
        if (row) {
            ...
            <div style="text-align:center; padding:25px 15px; font-family:Arial, sans-serif; background: transparent; color:#e0e7ff;">
                <div style="font-size:18px; font-weight:bold; color:#ffffff; text-shadow: 0 0 8px rgba(99,102,241,0.8);">C & Grab</div>
                <div style="margin-top:8px; font-size:13px; color:#a5b4fc;">Powered by <span style="font-weight:bold; color:#c7d2fe;">Pleiadians of Atlantis</span></div>
            </div>`;
    ...
    } catch (e) { console.error("CNG Engine Error:", e); }
}

// 5. AUTO-INVOKE IF PID FOUND IN GLOBAL SCOPE OR DATA-ATTRIBUTE (Optional)
