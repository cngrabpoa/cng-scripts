/**
 * CNG OFFICIAL PRODUCT LOADER - VERSION 5.0 (ULTRA FIX)
 * All Columns Mapped & Variables Synced.
 */
...
window.loadUltraProduct = async function(pid) {
    ...
    if (!document.getElementById('cng-ultra-styles')) {
        const css = `
            #master-luxury-root { font-family: 'Segoe UI', Roboto, sans-serif; color: #e2e8f0; line-height: 1.6; }
            ...
            .luxury-title { font-size: 30px; font-weight: 800; text-align: center; color: #fff; margin-bottom: 15px; }
        ...
        if (row) {
            ...
            const finalHTML = `
                <div class="ultra-luxury-card">
                    ...
                    ${renderUI("User Guide", p.manual)}
                    ${(p.sBox && p.sBox !== "/i") ? `<div class="safety-box">⚠️ ${p.sBox}</div>` : ''}
                    ${renderUI("Safety Info", p.sInfo)}
                    ...
                    <div style="text-align:center; margin-top:40px; border-top:1px solid #1e293b; padding-top:20px;">
                        ...
                        <small style="color:#64748b;">Powered by Pleiadians of Atlantis</small>
                    </div>
                </div>`;
